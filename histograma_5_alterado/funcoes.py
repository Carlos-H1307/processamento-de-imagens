...
def arredondarNormalizado(histograma):
    histAux = numpy.zeros( 256 )
    for i in range( 256 ):
        temp = round( histograma )
        if temp > 1:    
            temp = 1
        histAux[i] = temp
...
def normalizarHistograma(histograma, totalPixels):
    histAux = numpy.zeros( 256 )
    for i in range(256):
        histAux[i] = histograma[i] / totalPixels
    return histAux
...
def mapear4(equalizado, acumulado):
    mapa2 = numpy.zeros( 256 )
    print(len(equalizado))
    for valor in range( len(equalizado) ):
        menor = abs( equalizado[valor] - acumulado[0] )
        menorIndice = 0
        for j in range( len(acumulado) ):
            if abs(equalizado[valor]  - acumulado[j]) < menor:
                menor = abs(equalizado[valor] - acumulado[j])
                menorIndice = j
        mapa2[valor] = menorIndice
